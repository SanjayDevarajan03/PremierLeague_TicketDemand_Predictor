...
def average_demand_last_4_weeks(X: pd.DataFrame) -> pd.DataFame:
    """
    Adds one columns with the average rides from
    - 7 days
    - 14 days
    - 21 days
    - 28 days
...
class TemporalFeaturesEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        X_['is_weekend'] = X_['date'].isin([6, 7]).astype(int)
...
def get_pipeline(**hyperparams) -> Pipeline:
    ...
    return make_pipeline(add_feature_average_demand_last_4_weeks, add_temporal_features, lgb.LGBMRegressor(**hyperparams))
